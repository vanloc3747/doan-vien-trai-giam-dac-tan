    ...
    <div className="max-w-lg space-y-6">
      <div className="rounded-xl bg-white p-5 shadow-sm">
        ...
        <div className="space-y-1 text-sm text-slate-600">
          <div>Tên đăng nhập: <span className="font-medium text-slate-800">{user?.username}</span></div>
          <div>Họ và tên: <span className="font-medium text-slate-800">{user?.fullName}</span></div>
          <div>Vai trò: <span className="font-medium text-slate-800">{user?.role === 'admin' ? 'Quản trị viên' : 'Cán bộ đoàn'}</span></div>
        </div>
      </div>
