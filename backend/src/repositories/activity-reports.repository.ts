...
function mapRow(row: any) {
  return {
    ...
    reportedByManagedChapterId: row.reporter_managed_chapter_id,
    reportedByManagedChapterName: row.reporter_managed_chapter_name,
...
const SELECT_BASE = `
  SELECT ar.*, ap.title AS plan_title,
         u.managed_chapter_id AS reporter_managed_chapter_id,
         mc.name AS reporter_managed_chapter_name,
         COALESCE(
  ...
  LEFT JOIN chapters mc ON mc.id = u.managed_chapter_id
...
export async function listActivityReports(planId?: number) {
  ...
  const result = await pool.query(
    `${SELECT_BASE} ${whereClause} GROUP BY ar.id, ap.title, u.managed_chapter_id, mc.name
...
export async function getActivityReportById(id: number) {
  const result = await pool.query(
    `${SELECT_BASE} WHERE ar.id = $1 GROUP BY ar.id, ap.title, u.managed_chapter_id, mc.name`,
    [id]
  );
